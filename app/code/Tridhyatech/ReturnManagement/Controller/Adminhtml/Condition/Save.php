...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function __construct(
        ...
        \Tridhyatech\ReturnManagement\Model\ConditionFactory $conditionFactory
    ) {
    ...
    public function execute()
    {
        ...
        if (!$data) {
            $this->_redirect('ttrma/condition/form');
            return;
        }
        $currentDate = $this->date->gmtDate('Y-m-d H:i:s');
        $data['created_date'] = $currentDate;
        
        $rowData = $this->conditionFactory->create();
        ...
        if ($rowData->save()) {
            $this->messageManager->addSuccess(__('Condition has been successfully saved.'));
        } else {
